...
flowchart TB
    subgraph Client["Frontend (Angular)"]
        A[Navbar] --> B[Search Bar]
        A --> C[Login Button]
        A --> D[Basket Icon]
        B --> E[Product Cards]
    end
    subgraph Server["Backend (Express.js)"]
        F[Auth API] --> G[Products API]
        F --> H[Basket API]
        F --> I[User API]
    end
...
*Figura 1: Arquitectura de OWASP Juice Shop — SPA Angular + API REST Express + SQLite*
...
```mermaid
gantt
    title Juice Shop Challenge Progress
    dateFormat  X
    axisFormat %s
    section Easy (1-2★)
    Login Admin      :done, 0, 10
    DOM XSS          :done, 10, 20
    IDOR Basket      :done, 20, 30
    section Medium (3-4★)
    Reflected XSS    :active, 30, 40
    Directory Traversal :active, 40, 50
    section Hard (5-6★)
    JWT Tampering    :crit, 50, 60
    API XSS          :crit, 60, 70
```

*Figura 2: Score Board — Progreso de challenges por dificultad*
...
sequenceDiagram
    ...
    U->>B: Email: ' OR 1=1--
    U->>B: Password: anything
    B->>S: POST /rest/user/login
    S->>D: SELECT * FROM Users WHERE email='' OR 1=1--'
    D-->>S: Returns ALL users (first=admin)
    S-->>B: 200 OK + JWT (role:admin)
    B-->>U: Logged in as admin@juice-sh.op
    Note over U,D: SQL Injection bypasses authentication
```

*Figura 3: Flujo de ataque SQLi — Login Admin bypass*
...
flowchart TB
    ...
    subgraph Payload["Payload (Base64)"]
        B1[{"data":{"email":"user@juice-sh.op"}}]
        B2[{"role":"customer"}]
        B3[{"iat":1234567890}]
    end
    subgraph Signature["Signature (HMAC)"]
        C1[HMAC-SHA256(header.payload, secret)]
    end
    Header -->|.| Payload -->|.| Signature
    classDef h fill:#3b82f6,stroke:#1e40af,color:#fff
...
*Figura 8: Estructura JWT — Header, Payload, Signature*
...
```mermaid
flowchart TB
    subgraph Original["Original JWT"]
        A1[alg: HS256] --> A2[email: user@juice-sh.op]
        A2 --> A3[role: customer]
        A3 --> A4[Valid Signature]
    end
    subgraph Forged["Forged JWT (alg:none)"]
        B1[alg: none] --> B2[email: jwtn3d@juice-sh.op]
        B2 --> B3[role: admin]
        B3 --> B4[Signature: (empty)]
    end
    subgraph Result["Server Accepts"]
        C1[No signature verification]
        C2[User impersonated as admin]
    end
    Forged --> Result
    classDef orig fill:#3b82f6,stroke:#1e40af,color:#fff
    classDef forged fill:#ef4444,stroke:#b91c1c,color:#fff
    classDef result fill:#10b981,stroke:#047857,color:#fff
    class A1,A2,A3,A4 orig
    class B1,B2,B3,B4 forged
    class C1,C2 result
```

*Figura 9: JWT alg:none — Token forjado sin firma*
...
```mermaid
flowchart LR
    A[Authenticated User] --> B[GET /rest/basket/5]
    B --> C{Server checks ownership?}
    C -->|NO| D[Returns basket data]
    C -->|YES| E[403 Forbidden]
    D --> F[Attacker sees other user's cart]
    classDef attacker fill:#ef4444,stroke:#b91c1c,color:#fff
    classDef server fill:#f59e0b,stroke:#b45309,color:#fff
    classDef vuln fill:#10b981,stroke:#047857,color:#fff
    class A,B attacker
    class C,D server
    class E,F vuln
```

*Figura 10: IDOR — Enumeración de carritos sin verificación de ownership*
...
```mermaid
flowchart TB
    A[Request: /ftp/eastere.gg] --> B[Server serves /ftp/ directory]
    A2[Request: /ftp/../../../etc/passwd] --> C{Path validation?}
    C -->|Weak| D[Traversal succeeds]
    C -->|Strong| E[403 Forbidden]
    D --> F[Attacker reads system files]
    classDef req fill:#ef4444,stroke:#b91c1c,color:#fff
    classDef check fill:#f59e0b,stroke:#b45309,color:#fff
    classDef breach fill:#10b981,stroke:#047857,color:#fff
    class A,A2 req
    class C,D check
    class E,F breach
```

*Figura 11: Directory Traversal — Escape de directorio restringido*
